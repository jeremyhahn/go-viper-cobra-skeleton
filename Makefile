...
clone: clean
	cp -R ./ $(CLONE_DIR)
	grep -rl github.com/jeremyhahn $(CLONE_DIR) | xargs sed -i s@github.com/jeremyhahn@$(CLONE_HOST)/$(CLONE_OWNER)@g
	grep -rl $(PACKAGE) $(CLONE_DIR) | xargs sed -i s@$(PACKAGE)@$(CLONE_PACKAGE)@g
	grep -rl $(APPNAME) $(CLONE_DIR) | xargs sed -i s@$(APPNAME)@$(CLONE_APP_NAME)@g
	head -n -23 $(CLONE_DIR)/Makefile > $(CLONE_DIR)/Makefile.tmp
	mv $(CLONE_DIR)/Makefile.tmp $(CLONE_DIR)/Makefile
	echo "" > $(CLONE_DIR)/LICENSE
	rm -rf $(CLONE_DIR)/.git
	echo "0.0.0-alpha.1" > $(CLONE_DIR)/$(VERSION_FILE)
	cd $(CLONE_DIR) && \
		git init && \
		git add -A && \
		git commit -m "initial commit" && \
		git log
	@echo "${GREEN}Successfully create new skeleton project: $(CLONE_PACKAGE)${NO_COLOR}\n"
